...
export default function FilterEventsDropDown(props) {
    ...
    const handleChange = (event) => {
        ...
        selectedValue = selectedValue.filter(((d) => { return value.includes(d.event) }));
    ...
    return (
        <FormControl sx={{ width: DROPDOWN_WIDTH }} variant="standard" size="small">
            ...
            <Select
                ...
                renderValue={(events) => { return events.join(', '); }}
                MenuProps={MenuProps}
            >
                {selectedValue.map((value) => (
                    <MenuItem key={value.event} value={value.event}>
                        <Checkbox checked={events.indexOf(value.event) > -1} />
                        <ListItemText primary={value.event + " : " + durToSec(value.dur, 2) + "s"} />
                    </MenuItem>
                ))}
            </Select>
        </FormControl>
